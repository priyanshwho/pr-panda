...
export async function deleteInstallation(userId: string) {
    ...
    if (installation) {
        try {
            ...
            console.error("Failed to delete installation from GitHub:", error);
            // We still proceed to delete from the database even if GitHub API fails
            // (e.g., if it was already deleted on GitHub)
        }
