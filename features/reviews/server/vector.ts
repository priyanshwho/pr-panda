...
const CONTEXT_RESULTS = 10; //how many similar chunks we want to fetch from the pinecone db
...
export async function searchPrContext(namespace: string, query: string) {
    //symentic search through pinecone db then feed ai with similar chunks to generate better response
    const index = getPineconeIndex();
